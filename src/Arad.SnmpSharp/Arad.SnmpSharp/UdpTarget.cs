...
public class UdpTarget: UdpTransport, IDisposable
{
    ...
    public SnmpPacket Request(Pdu pdu, IAgentParameters agentParameters)
    {
        ...
        if (agentParameters.Version == SnmpVersion.Ver3)
        {
            ...
            ScopedPdu outPdu = new(pdu);
            SnmpV3Packet packet = new(outPdu);
        ...
        else if (agentParameters.Version == SnmpVersion.Ver1)
        {
            ...
            if (!param.Valid())
                throw new SnmpException.SnmpException(SnmpException.SnmpException.InvalidIAgentParameters,"Invalid AgentParameters. Unable to process request.");
            SnmpV1Packet packet = new();
            packet.Pdu.Set(pdu);
        ...
        {
            throw new SnmpException.SnmpException(SnmpException.SnmpException.NoDataReceived, "No data received on request.");
        }
        // verify packet
        if (agentParameters.Version == SnmpVersion.Ver1)
        {
            ...
            if (packet.Pdu.RequestId != pdu.RequestId)
            {
                ...
                throw new SnmpException.SnmpException(SnmpException.SnmpException.InvalidRequestId, "Invalid request id in reply.");
            }
            return packet;
        }
        else if (agentParameters.Version == SnmpVersion.Ver2)
    ...
    public bool RequestAsync(Pdu pdu, IAgentParameters agentParameters, SnmpAsyncResponse responseCallback)
    {
        ...
        if( ! base.RequestAsync(_address, _port, outPacket, outPacket.Length, _timeout, _retry, new(AsyncResponse) ) ) {
            return false;
        }
        return true;
    }
    ...
    public bool Discovery(SecureAgentParameters param)
    {
        ...
        Pdu pdu = new(); // just leave everything at default.
