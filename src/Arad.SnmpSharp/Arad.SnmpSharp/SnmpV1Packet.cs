...
public class SnmpV1Packet : SnmpPacket
{
    ...
    {
        MutableByte buf = new(buffer, length);
        ...
        if (asnType != (byte)PduType.Get && asnType != (byte)PduType.GetNext && asnType != (byte)PduType.Set && asnType != (byte)PduType.Response)
            throw new SnmpInvalidPduTypeException("Invalid SNMP operation received: " + $"0x{asnType:x2}");
    ...
    public override byte[] encode()
    {
        if (this.Pdu.Type != PduType.Get && this.Pdu.Type != PduType.GetNext && 
            ...
            throw new SnmpInvalidVersionException("Invalid SNMP PDU type while attempting to encode PDU: " + $"0x{this.Pdu.Type:x2}");
        if (this.Pdu.RequestId == 0)
        {
            System.Random rand = new((System.Int32)DateTime.Now.Ticks);
            this.Pdu.RequestId = rand.Next();
        }
        MutableByte tmpBuffer = new();
        // snmp version
