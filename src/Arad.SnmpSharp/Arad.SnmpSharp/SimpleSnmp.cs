...
using Oid = Arad.SnmpSharp.Types.Oid;
...
public class SimpleSnmp
{
    ...
    public Dictionary<Oid, AsnType> Get(SnmpVersion version, Pdu pdu)
    {
        if (!Valid)
        {
            ...
            {
                throw new SnmpException.SnmpException("SimpleSnmp class is not valid.");
            }
        ...
        try
        {
            _target = new(_peerIP, _peerPort, _timeout, _retry);
        }
        catch( Exception ex )
        {
            _target = null;
        ...
        try
        {
            AgentParameters param = new(version, new(_community));
            SnmpPacket result = _target.Request(pdu, param);
            if (result != null)
            {
                if (result.Pdu.ErrorStatus == 0)
                {
                    Dictionary<Oid, AsnType> res = new();
                    foreach (Vb v in result.Pdu.VbList)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    throw new SnmpException.SnmpException(SnmpException.SnmpException.OidValueTypeChanged, $"Value type changed from {res[v.Oid].Type} to {v.Value.Type}");
                                }
                            }
                        }
                    }
                ...
                if (result.Pdu.ErrorStatus == 0)
                {
                    ...
                    foreach (Vb v in result.Pdu.VbList)
                    {
                        ...
                        if (res.ContainsKey(v.Oid))
                        {
                            if (res[v.Oid].Type != v.Value.Type)
                            {
                                throw new SnmpException.SnmpException(SnmpException.SnmpException.OidValueTypeChanged, "OID value type changed for OID: " + v.Oid);
                            }
                            else
                            {
                                res[v.Oid] = v.Value;
    ...
    public Dictionary<Oid, AsnType> GetBulk(Pdu pdu)
    {
        ...
        try
        {
            AgentParameters param = new(SnmpVersion.Ver2, new(_community));
            SnmpPacket result = _target.Request(pdu, param);
            if (result != null)
            {
                if (result.Pdu.ErrorStatus == 0)
    ...
    public Dictionary<Oid, AsnType> Walk(SnmpVersion version, string rootOid)
    {
        ...
        if (rootOid.Length < 2)
        {
            if( ! _suppressExceptions)
            {
                throw new SnmpException.SnmpException(SnmpException.SnmpException.InvalidOid, "RootOid is not a valid Oid");
            }
            return null;
        }
        Oid root = new(rootOid);
        if (root.Length <= 0)
        {
        ...
        Dictionary<Oid, AsnType> result = new();
            ...
            foreach (KeyValuePair<Oid, AsnType> entry in val)
            {
                if (root.IsRootOf(entry.Key))
                {
                    if (result.ContainsKey(entry.Key))
                    {
                        ...
                        {
                            throw new SnmpException.SnmpException(SnmpException.SnmpException.OidValueTypeChanged, "OID value type changed for OID: " + entry.Key);
                        }
                        else
