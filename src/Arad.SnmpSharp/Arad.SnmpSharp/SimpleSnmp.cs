						...
						foreach (Vb v in result.Pdu.VbList)
						{
							...
							else
							{
								...
								else
								{
									...
									{
										throw new SnmpException.SnmpException(SnmpException.SnmpException.OidValueTypeChanged, string.Format("Value type changed from {0} to {1}", res[v.Oid].Type, v.Value.Type));
									}
								}
							}
						}
					...
					if (result.Pdu.ErrorStatus == 0)
					{
						...
						foreach (Vb v in result.Pdu.VbList)
						{
							...
							if (res.ContainsKey(v.Oid))
							{
								if (res[v.Oid].Type != v.Value.Type)
								{
									throw new SnmpException.SnmpException(SnmpException.SnmpException.OidValueTypeChanged, "OID value type changed for OID: " + v.Oid.ToString());
								}
								else
								{
									res[v.Oid] = v.Value;
				...
				foreach (KeyValuePair<Oid, AsnType> entry in val)
				{
					if (root.IsRootOf(entry.Key))
					{
						if (result.ContainsKey(entry.Key))
						{
							...
							{
								throw new SnmpException.SnmpException(SnmpException.SnmpException.OidValueTypeChanged, "OID value type changed for OID: " + entry.Key.ToString());
							}
							else
