...
public class Vb : AsnType, ICloneable
{
    ...
    public override void encode(MutableByte buffer)
    {
        ...
        MutableByte oidbuf = new();
        _oid.encode(oidbuf);
        // encode value to a temporary buffer
        MutableByte valbuf = new();
    ...
    public override int decode(byte[] buffer, int offset)
    {
        ...
        if (asnType != Type)
            throw new SnmpException.SnmpException($"Invalid ASN.1 type. Expected 0x{Type:x2} received 0x{asnType:x2}");
        ...
        if (_value == null)
            throw new SnmpDecodingException($"Invalid ASN.1 type encountered 0x{asnType:x2}. Unable to continue decoding.");
