...
public class SecureAgentParameters : IAgentParameters
{
    ...
    public bool ValidateIncomingPacket(SnmpV3Packet packet)
    {
        ...
        if (packet.Pdu.Type == PduType.Report)
        {
            if (!_reportable)
            {
                ...
                throw new SnmpException.SnmpException(SnmpException.SnmpException.ReportOnNoReports, "Unexpected report packet received.");
                // return false; 
            }
            if (packet.MsgFlags.Authentication == false && packet.MsgFlags.Privacy)
            {
                ...
                throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedNoAuthPriv, "Authentication and privacy combination is not supported.");
                // return false; 
            }
        ...
        else
        {
            if (packet.USM.EngineId != _engineId)
            {
                ...
                throw new SnmpException.SnmpException(SnmpException.SnmpException.InvalidAuthoritativeEngineId, "EngineId mismatch.");
                // return false; 
            }
            ...
            if (packet.USM.Authentication != _authenticationProtocol || packet.USM.Privacy != _privacyProtocol)
            {
                ...
                throw new SnmpException.SnmpException("Agent parameters updated after request was made.");
                // return false; 
            }
            ...
            {
                throw new SnmpException.SnmpException(SnmpException.SnmpException.InvalidSecurityName, "Security name mismatch.");
                // return false;
            }
        }
        return true;
    ...
    public void Reset()
    {
        ...
        _privacySecret = new();
        _authenticationSecret = new();

        _contextEngineId = new();
        _contextName = new();
        _securityName = new();
    ...
    public void BuildCachedSecurityKeys()
    {
        ...
        if (_authenticationProtocol != AuthenticationDigests.None)
        {
            ...
            if (authProto != null)
            {
                ...
                {
                    IPrivacyProtocol privProto = Security.PrivacyProtocol.GetInstance(_privacyProtocol);
                    if (privProto != null)
                    {
