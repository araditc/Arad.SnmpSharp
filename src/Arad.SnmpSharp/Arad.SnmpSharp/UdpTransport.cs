...
public class UdpTransport: IDisposable
{
    ...
    protected void initSocket(bool useV6)
    {
        ...
        if (useV6)
        {
            _socket = new(AddressFamily.InterNetworkV6, SocketType.Dgram, ProtocolType.Udp);
        }
        else
        {
            _socket = new(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
        }
        IPEndPoint ipEndPoint = new(_socket.AddressFamily == AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, 0);
    ...
    public byte[] Request(IPAddress peer, int port, byte[] buffer, int bufferLength, int timeout, int retries)
    {
        ...
        if (_socket.AddressFamily != peer.AddressFamily)
            throw new SnmpException.SnmpException("Invalid address protocol version.");

        IPEndPoint netPeer = new(peer, port);
        ...
        while (true)
        {
            ...
            if (recv > 0)
            {
                ...
                if ( ! _noSourceCheck && ! remEP.Equals(netPeer))
                {
                    ...
                    if (retry > retries)
                    {
                        throw new SnmpException.SnmpException(SnmpException.SnmpException.RequestTimedOut, "Request has reached maximum retries.");
                        // return null;
                    }
                }
                else
                {
                    MutableByte buf = new(inbuffer, recv);
                    return buf;
                }
            }
            else
    ...
    internal bool RequestAsync(IPAddress peer, int port, byte[] buffer, int bufferLength, int timeout, int retries, SnmpAsyncCallback asyncCallback)
    {
        ...
        _requestState = new(peer, port, retries, timeout);
    ...
    internal void SendToBegin()
    {
        ...
        if (_socket == null)
        {
            _busy = false;
            _requestState = null;
            _asyncCallback(AsyncRequestResult.Terminated, new(IPAddress.Any, 0), null, 0);
            return; // socket has been closed. no new operations are possible.
        }
        try
        {
            _socket.BeginSendTo(_requestState.Packet, 0, _requestState.PacketLength, SocketFlags.None, _requestState.EndPoint, new(SendToCallback), null);
        }
        catch
        {
            _busy = false;
            _requestState = null;
            _asyncCallback(AsyncRequestResult.SocketSendError, new(_socket.AddressFamily == AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, 0), null, 0);
        }
    }
    /// <summary>
    ...
    internal void SendToCallback(IAsyncResult ar)
    {
        ...
        catch (NullReferenceException ex)
        {
            ...
            _busy = false;
            _requestState = null;
            _asyncCallback(AsyncRequestResult.Terminated, new(_socket.AddressFamily == AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, 0), null, 0);
            return;
        }
        catch
        {
    ...
    internal void ReceiveBegin()
    {
        ...
        _receivePeer = new(_socket.AddressFamily == AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, 0);
        EndPoint ep = _receivePeer;
        try
        {
            _socket.BeginReceiveFrom(_inBuffer, 0, _inBuffer.Length, SocketFlags.None, ref ep, new(ReceiveFromCallback), null);
        }
        catch
        {
        ...
        _requestState.Timer = new(new(AsyncRequestTimerCallback), null, _requestState.Timeout, System.Threading.Timeout.Infinite);
    }
    /// <summary>
    ...
    internal void RetryAsyncRequest()
    {
        ...
        if (_requestState.CurrentRetry >= _requestState.MaxRetries)
        {
            _busy = false;
            _requestState = null;
            _asyncCallback(AsyncRequestResult.Timeout, new(_socket.AddressFamily == AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, 0), null, 0);
            return;
        }
        else
        {
