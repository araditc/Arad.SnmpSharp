...
public class SnmpV3Packet: SnmpPacket
{
    ...
    {
        MutableByte buffer = new(berBuffer, length);
        ...
        if (_msgFlags.Authentication == false && _msgFlags.Privacy == true)
            throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedNoAuthPriv,
        ...
        if (_securityModel.Value != _userSecurityModel.Type)
            throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedSecurityModel,
    ...
    public override int decode(byte[] berBuffer, int length)
    {
        ...
        if (_msgFlags.Authentication && _userSecurityModel.EngineId.Length > 0)
        {
            ...
            if (auth == null)
                throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedNoAuthPriv, "Invalid authentication protocol.");
            ...
            if (_msgFlags.Privacy && _userSecurityModel.EngineId.Length > 0)
            {
                ...
                if (privacyProtocol == null)
                    throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedPrivacyProtocol, "Specified privacy protocol is not supported.");
    ...
    public int decode(byte[] berBuffer, int length, byte[] authKey, byte[] privKey)
    {
        ...
        // verify that a valid authentication/privacy configuration is present in the packet
        if (_msgFlags.Authentication == false && _msgFlags.Privacy == true)
            throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedNoAuthPriv, "SNMP version 3 noAuthPriv security combination is not supported.");
        ...
        // we only support USM. code = 0x03
        if (_securityModel.Value != _userSecurityModel.Type)
            throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedSecurityModel, "Class only support SNMP Version 3 User Security Model.");
        ...
        if (_msgFlags.Privacy && _userSecurityModel.EngineId.Length > 0)
        {
            ...
            if (privacyProtocol == null)
            {
                throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedPrivacyProtocol, "Privacy protocol requested is not supported.");
            }
            if (_userSecurityModel.PrivacyParameters.Length != privacyProtocol.PrivacyParametersLength)
                throw new SnmpException.SnmpException(SnmpException.SnmpException.InvalidPrivacyParameterLength, "Invalid privacy parameters field length.");

            // Initialize a temporary Octetstring class to hold encrypted ScopedPdu
            Octetstring encryptedScopedPdu = new();
    ...
    public byte[] encode(byte[] authKey, byte[] privKey)
    {
        ...
        MutableByte packetHeader = new(buffer);

        // before going down this road, check if this is a discovery packet
        Octetstring savedUserName = new();
        ...
        if (_msgFlags.Privacy && _userSecurityModel.EngineId.Length > 0)
        {
            ...
            MutableByte unencryptedPdu = new();
            ...
            IAuthenticationDigest auth = Authentication.GetInstance(_userSecurityModel.Authentication);
            if (auth == null)
                throw new SnmpException.SnmpException(SnmpException.SnmpException.UnsupportedNoAuthPriv, "Invalid authentication protocol. noAuthPriv mode not supported.");
            ...
            Octetstring encryptedOctetstring = new(encryptedBuffer);
            ...
            if (_maxMessageSize.Value != 0)
            {
                ...
                {
                    throw new SnmpException.SnmpException(SnmpException.SnmpException.MaximumMessageSizeExceeded, "ScopedPdu exceeds maximum message size.");
                }
            }
        }
        else
    ...
    {
        SnmpV3Packet packet = new(new ScopedPdu()); // with a blank scoped pdu
    ...
    public static SnmpV3Packet BuildInformResponse(SnmpV3Packet informPacket)
    {
        ...
        SnmpV3Packet response = new(informPacket.ScopedPdu);
